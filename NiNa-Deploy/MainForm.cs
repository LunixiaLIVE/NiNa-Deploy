...
namespace NiNa_Deploy
{
    public partial class AppForm : Form
    {
        ...
        {
            InitializeComponent();
            this.Icon = new Icon($@"{AppDomain.CurrentDomain.BaseDirectory}\Resources\Ninja.ico");
            ...
            if (File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}\Resources\PsExec64.exe"))
            {
                tb_PsExecLocation.Text = $@"{AppDomain.CurrentDomain.BaseDirectory}\Resources\PsExec64.exe";
            }
            else
            {
        ...
        void ContinuousDoEvents()
        {
            ...
            {
                Thread.Sleep(100);
                if (lbl_MonitoringThreads.IsDisposed) return;
        ...
        private void aboutToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (String s in File.ReadAllLines($@"{AppDomain.CurrentDomain.BaseDirectory}\Resources\AuthorInfo.txt"))
            {
                sb.Append($"{s}\r\n");
            }
            MessageBox.Show($"{sb}" +
        ...
        private void DoThreadThings(String strComputer, String cmdLine, String tempFolder, String packagePath, String psExecPath, bool asSystem, bool interactive, int pingTimeout)
        {
            Console.WriteLine($"Remote Thread started for: {strComputer}");

            lock (Utility.TableLock)
            {
            ...
            try
            {
                PingReply pingReply = ping.Send(strComputer, pingTimeout, buffer, pingOptions);
            ...
            lock (Utility.TableLock)
            {
                for (int i = 0; i < Utility.RemoteProcessesResults.Rows.Count; i++)
                {
                    if (Utility.RemoteProcessesResults.Rows[i]["RemoteHost"].ToString() == strComputer)
                    {
                        Utility.RemoteProcessesResults.Rows[i]["CMD"] = cmdLine;
                        break;
                    }
                }
            }
            ...
            ///PREP - FILE PREP
            ///
            String rD = $@"\\{strComputer}\{tempFolder.Replace("C:", "C$")}";
            String fN = packagePath.Split(@"\").Last();
            ...
            if (!File.Exists($@"{rD}\{fN}"))
            {
                try
                {
                    File.Copy(packagePath, $@"{rD}\{fN}", true);
                }
                catch (Exception e)
                {
                    for (int i = 0; i < Utility.RemoteProcessesResults.Rows.Count; i++)
            ...
            ///EXECUTE - PROCESS
            ///
            String Arguments = $@"\\{strComputer} -accepteula -s -i {cmdLine}";
            if (!asSystem) { Arguments = Arguments.Replace(" -s", ""); }
            if (!interactive) { Arguments = Arguments.Replace(" -i", ""); }
            ...
            lock (Utility.TableLock)
            {
                for (int i = 0; i < Utility.RemoteProcessesResults.Rows.Count; i++)
                {
                    if (Utility.RemoteProcessesResults.Rows[i]["RemoteHost"].ToString() == strComputer)
                    {
                        Utility.RemoteProcessesResults.Rows[i]["PID"] = pid;
                        break;
                    }
                }
            }
        ...
        private void btn_Execute_Click(object sender, EventArgs e)
        {
            ...
            String cmdLine     = tb_CommandLine.Text;
            String tempFolder  = tb_RemoteTempFolder.Text;
            String packagePath = tb_PackageLocation.Text;
            String psExecPath  = tb_PsExecLocation.Text;
            bool   asSystem    = chk_PsExec_System.Checked;
            bool   interactive = chk_PsExecRemoteInteractive.Checked;
            int    pingTimeout = (int)num_PingTimeout.Value;
            foreach (String strComputer in list_TargetComputers.Items)
            {
                ...
                Thread thread = new(() => DoThreadThings(strComputer, cmdLine, tempFolder, packagePath, psExecPath, asSystem, interactive, pingTimeout));
                thread.Start();
